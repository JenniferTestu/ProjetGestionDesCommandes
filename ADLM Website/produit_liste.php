<?php 
	...
	if (isset($_GET['souscategorie']) && isset($_GET['categorie'])) {
	
		$souscategorie = $_GET['souscategorie'];
		$categorie = $_GET['categorie'];
	
		$retour_total=mysqli_query($con,'SELECT COUNT(DISTINCT `id_produit`) AS total FROM produits, stocks WHERE souscategorie = "'.$souscategorie.'" AND categorie = "'.$categorie.'" AND id = id_produit AND nombre > 0'); // On compte le nombre de produits de la categorie voulu et qui existent dans les stocks 
		...
		$retour_messages=mysqli_query($con,'SELECT * FROM produits WHERE souscategorie = "'.$souscategorie.'" AND categorie = "'.$categorie.'" ORDER BY id DESC LIMIT '.$premiereEntree.', '.$messagesParPage.'');
		...
		for($i=1; $i<=$nombreDePages; $i++)
		{
			 ...
			 {
				  $pagination .= ' <a href="produit_liste.php?categorie='.$categorie.'&souscategorie='.$souscategorie.'&page='.$i.'">'.$i.'</a> ';
			 }
		}
		echo '</p>';
	}
